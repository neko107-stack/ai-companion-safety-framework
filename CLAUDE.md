...
思考の丸投げ（認知的オフロード）を防ぎ、ユーザー自身の考える力を育てる仕組み。`sendMessage()` が発話を解析し `extra` ディレクティブを動的注入する。
...
### 利用者向け文言（ウェルカム／オンボーディング／プラン選択）
判定則は **「利用者に起きること」は書く／「使っている道具の名前」は書かない**。C-SSRS・CBT・DBT 等の臨床評価尺度名や AES-256-GCM などの技術語をウェルカム画面に追加しないこと（技術語は、利用者がその場で判断を下すエクスポート・暗号化設定の画面にのみ置く）。これは commit `0d68d87`「医療的標榜を廃止し『友達アプリ』ポジショニングに統一」で確定した方針で、逆行させる変更は README / 法務文書 / 医療機器非該当の立場にも波及する。根拠と判定表は `SAFETY_FRAMEWORK.md` 5.8。
