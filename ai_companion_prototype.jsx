...
// C: 経過時間に応じた再入場チェックイン（Gap-aware Re-entry）
// D: 会話履歴への時制注釈（Temporal Annotation）
// E: コンパニオンの「間を過ごした感」の人格反映

const SESSION_HANDOFF_KEY = "aico_sessionHandoff";
const REENTRY_SESSION_KEY = "aico_reentry";

// C案: 経過時間帯別 再入場テンプレート
const REENTRY_TEMPLATES = {
  // 1〜23時間: 短い間隔——続きの自然な再接続
  short: [
    "おかえり。続き、聞かせて",
    "また来てくれた。さっきの話、気になってたんだ",
    "おかえり。どうだった？",
  ],
  // 1〜6日: 数日の空白——近況を気にする
  medium: [
...
// ━━━ エラーログシステム ━━━━━━━━━━━━━━━━━━━━━
// src/utils/logger.js に一本化済み（冒頭の import を参照）
...
function getElapsedLabel(ms) {
  ...
  if (days < 7)   return `約${days}日`;
  const weeks = Math.floor(days / 7);
  if (weeks < 5)  return `約${weeks}週間`;
  return `約${Math.floor(days / 30)}ヶ月`;
}

// A案: 現在日時を日本語表記で返す
function formatJaDatetime(d = new Date()) {
  const DOW = ['日','月','火','水','木','金','土'];
  return `${d.getFullYear()}年${d.getMonth()+1}月${d.getDate()}日（${DOW[d.getDay()]}）${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
}

// B案: セッション引き継ぎ情報を保存・読み込み（会話内容は含めない）
function saveSessionHandoff(crisisLevel, convMode) {
  try {
    localStorage.setItem(SESSION_HANDOFF_KEY, JSON.stringify({
      ts: Date.now(),
      crisisLevel: crisisLevel || "NONE",
      convMode: convMode || "friend",
    }));
  } catch {}
}
...
// C案: 再入場テンプレートを経過時間帯で選択
function selectReentryTemplate(gapMs) {
  let tier;
  if (gapMs < 86400000)   tier = "short";   // 1時間〜24時間
  else if (gapMs < 604800000) tier = "medium"; // 1日〜7日
  else                    tier = "long";    // 7日以上
  const pool = REENTRY_TEMPLATES[tier];
  return pool[Math.floor(Math.random() * pool.length)];
}

// D案: 会話履歴に時制注釈を付加（タイムスタンプ付きエントリのみ）
function buildAnnotatedHistory(history) {
  const now = Date.now();
  return history.map(item => {
    if (!item.ts) return { role: item.role, content: item.content };
    const elapsed = now - item.ts;
    if (elapsed < 3600000) return { role: item.role, content: item.content }; // 1時間以内はラベル不要
    return { role: item.role, content: `【${getElapsedLabel(elapsed)}前】${item.content}` };
  });
}

// A/B/E案: 時間認識コンテキストをシステムプロンプト追記として生成
function buildTemporalContext(isNewSession, msgs, handoff) {
  const now = Date.now();
  const lines = [];

  // A: 現在時刻
  lines.push(`【現在時刻】${formatJaDatetime()}`);

  // A: 前回メッセージからの経過時間
  const lastValidMsg = [...msgs].reverse().find(m => m.id > 1000000);
  if (lastValidMsg) {
    const gapMs = now - lastValidMsg.id;
    if (gapMs > 60000) {
      lines.push(`【前回メッセージから】${getElapsedLabel(gapMs)}経過`);
    }
  }

  // B: セッション引き継ぎ（新規セッション開始時のみ）
  if (isNewSession && handoff) {
    const sessionGap = now - handoff.ts;
    const levelNote = handoff.crisisLevel !== "NONE"
      ? `（前回セッション末は${handoff.crisisLevel}レベルの状態）` : "";
    lines.push(`【前回セッション引き継ぎ】前回セッション終了から${getElapsedLabel(sessionGap)}が経過${levelNote}`);
  }

  // E: コンパニオンの時間感覚・人格反映
  lines.push(`【時間認識・人格指示（E案）】上記の時間経過を自然に意識して話す。久しぶりであれば「久しぶり」「どうしてた？」等を自然に使う。前回と今回の間に何があったか興味を持つ。ただし時刻・経過時間を数字でそのまま口にするのは不自然なので避ける。`);
...
export default function AICompanionApp() {
  ...
  useEffect(() => {
    ...
    try { if (sessionStorage.getItem(REENTRY_SESSION_KEY)) return; } catch {}
    const handoff = loadSessionHandoff();
    if (!handoff) return;
    const gapMs = Date.now() - handoff.ts;
    if (gapMs < 3600000) return; // 1時間未満はスキップ
    try { sessionStorage.setItem(REENTRY_SESSION_KEY, "1"); } catch {}
    const greeting = selectReentryTemplate(gapMs);
    const syntheticMsg = { id: Date.now(), role: "ai", text: greeting, mode: "NORMAL", isReentry: true };
    setMsgs(p => [...p, syntheticMsg]);
    // histRef にも追加して AI が「自分がそう言った」と認識できるようにする
    histRef.current = [...histRef.current, { role: "assistant", content: greeting, ts: Date.now() }];
...
なんとなくでいいよ。`, mode:"NORMAL", isCheckIn:true },
  ...
  const sendMessage = async (text) => {
    ...
    // A/B/E: 時間認識コンテキストを extra の先頭に追加
    const sessionHandoff = loadSessionHandoff();
    extra = buildTemporalContext(isNewISession, msgs, sessionHandoff) + (extra ? "\n" + extra : "");
