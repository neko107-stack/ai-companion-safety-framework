...
- **正規実装は `src/`**。`ai_companion_prototype.jsx`（231KB）は元プロトタイプだが、`src/main.jsx` がここから `AICompanionApp` を import しており、**UI とオーケストレーション層（`sendMessage()`・介入状態管理・認知成長スキャフォールディング）は現状この巨大ファイル内に存在する**。`src/` 配下の各モジュールはそこから呼び出される「部品」。
...
| `aico_msgs` | `localStorage` | AES-256-GCM | 会話履歴 |
